			...
			<div class="row d-flex justify-content-between" style="margin-top: 2vh;padding-left:1rem">
				<div class="col bays p-4">
					...
					<div class="row d-flex justify-content-between">
						<div class="col-4">
							...
							<div class="mb-3">
								...
								<select class="form-control selectpicker" name="typeso" required>
							...
							<div class="mb-3">
								...
								<select class=" form-control selectpicker" data-live-search="true" name="idcust" onchange="customer(this.value)" required>
						...
						<div class="col-4">
							...
							<div class="mb-3">
								...
								<input type="text" name="jasapengirim" class="form-control" placeholder="Pilih Jasa Pengiriman" required>
							</div>
							<div class="mb-3">
								<label>Biaya Pengiriman</label>
								<input type="text" name="delivfee" class="form-control" placeholder="Masukan Biaya Pengiriman">
							</div>
							<div class="mb-3">
							...
							<div class="mb-3">
								...
								<!-- <input type="text" name="delivaddr" id="alamat" class="form-control" placeholder="Alamat Disini" required> -->
								<textarea name="delivaddr" id="alamat" cols="30" rows="4" class="form-control" required></textarea>
							</div>
						</div>
						<div class="col-4">
							<div class="mb-3">
							...
							<div class="row d-flex justify-content-between mb-3">
								...
								<div class="col-6">
									...
									<input type="text" name="disnoms" id="disnoms" onclick="disnomsy()" oninput="count()" class="form-control" placeholder="Masukan Nominal" readonly>
								</div>
							</div>
							<div class="mb-3">
								...
								<div class="card-footer pb-4" style="background: #eeeeee;">
									...
									<div class="row d-flex justify-content-between">
										<div class="col">
											...
											<input type="text" id="hargaitemx" onclick="focusprice()" class="form-control" readonly>
										</div>
										<div class="col">
											<p>DPP</p>
					...
					<div class="d-flex" style="margin-top: 50px">
						...
						<input type="text" class="py-1" id="jmlitem" name="" value="0" style="border:0px ;margin-left: 20px;border-radius:4px">
					...
					<div class="bays">
						...
						<div style="padding: 20px">
							...
							<div class="row">
								...
								<div class="col-4">
									<input style="text-align: right;" type="text" name="qty" id="qtys" value="0" class="form-control" readonly>
								</div>
							</div>
							...
							<div class="row">
								...
								<div class="col-4">
									<input style="text-align: right;" type="text" name="subtotals" id="subtotals" value="0" class="form-control" readonly>
								</div>
							</div>
							...
							<div class="row">
								...
								<div class="col-4">
									<input style="text-align: right;" type="text" value="0" id="disnomx" class="form-control" readonly>
							...
							<div class="row" style="display: none;">
								...
								<div class="col-4">
									<input style="text-align: right;" type="text" id="vatnom" name="vatnom" value="0" class="form-control" readonly>
								</div>
							</div>
							<hr>
							<div class="row">
								<div class="col-4">
									<p class="fs-5 mt-2"><b>Grand Total</b></p>
								</div>
								<div class="col-8">
									<input type="text" style="font-weight: bold;text-align: right;" name="grandtotals" id="grandtotals" value="0" class="form-control fs-5 border-0 bg-transparent" readonly>
...
<script type="text/javascript">
	...
	input.addEventListener("keyup", function(event) {
		...
		if (event.keyCode === 13) {
			...
			// 	$('#vat').val($('#hargaitem').val() - ($('#hargaitem').val() / 1.1));
		...
		for (var i = 0; i < data.length; i++) {
			if (data[i]["iditem"] == x) {
				...
				if (data[i]["VAT"] != null && data[i]["VAT"] != 0) {
					$('#vat').val(data[i]["priceitem"] - (data[i]["priceitem"] / data[i]["VAT"]))
				} else {
					$('#vat').val(0)
				}
				...
				if (data[i]["subprice"] != null) {
					$('#subtotalx').val(data[i]["subprice"])
					// console.log(data[i]["subprice"])
				} else {
	...
	function tambah() {
		...
		if ($('#nameitem').val() == "") {
			...
			<tr id=item` + $('#jmlitemx').val() + ` style="style="height:40px;text-align:center">
										<td>` + b + `</td>
			<td style="text-align:center" id=nameitem[]><input type="hidden"  name="nameitemx[]" value ="` + $('#nameitem').val() + `"> <input type="hidden"  name="idwh[]" value ="` + $('#idwh').val() + `"> <input type="hidden"  name="iditemx[]" value="` + $('#iditem').val() + `">` + $('#nameitem').val() + `</td>
			<td style="text-align:center" id=sku[]> <input type="hidden"  name="skux[]" value ="` + $('#sku').val() + `">` + $('#sku').val() + `</td>
			<td style="text-align:center" id=hargaitem[]> <input type="hidden"  name="hargaitem[]" value ="` + $('#hargaitem').val() + `">` + formatRupiah($('#hargaitem').val(), "Rp. ") + `</td>
			<td style="text-align:center" id=dpp[]> <input type="hidden"  name="hargabeli[]" value ="` + $('#hargabeli').val() + `">  <input type="hidden"  name="dpps[]" value ="` + $('#dpp').val() + `">` + formatRupiah($('#dpp').val(), "Rp. ") + `</td>
			<td style="text-align:center" id=vat[]> <input type="hidden"  name="vat[]" value ="` + $('#vat').val() + `">` + $('#vatx').val() + `</td>
			<td style="text-align:center" id=qtyorder[]> <input type="hidden"  name="qtyorderx[]" value ="` + $('#qtyorder').val() + `"><input type="hidden"  name="qtyready[]" value ="` + $('#balance').val() + `">` + $('#qtyorder').val() + `</td>
			<td style="text-align:center" id=price[]> <input type="hidden"  name="pricex[]" value ="` + $('#subtotal').val() + `">` + formatRupiah($('#subtotalx').val(), "Rp. ") + `</td>
			<td style="text-align:center" id=disc[]> <input type="hidden"  name="discx[]" value ="` + $('#totaldiscount').val() + `">` + formatRupiah($('#totaldiscountx').val(), "Rp. ") + `</td>
			<td style="text-align:center" id=subtotal[]> <input type="hidden"  name="subtotalxx[]" value ="` + $('#grandtotal').val() + `">` + formatRupiah($('#grandtotalx').val(), "Rp. ") + `</td>
			<td style="text-align:center" id=Action[]><button class="btn btn-outline-danger" onclick="del(` + $('#jmlitemx').val() + `,` + $('#grandtotal').val() + `,` + (Number($('#subtotal').val() - ($('#hargaitem').val() * $('#qtyorder').val()))) + `,` + $('#totaldiscount').val() + `)">Delete</button></td>
					...
					baris += `
								<tr id=item` + $('#jmlitemx').val() + ` style="style="height:40px;text-align:center; background:#eeeeee">
										...
										<td style="text-align:center" id=hargaitem[]> <input type="hidden"  name="hargaitem[]" value ="` + $('#hargaitem').val() + `">` + formatRupiah($('#hargaitem').val(), "Rp. ") + `</td>
			<td style="text-align:center" id=dpp[]> <input type="hidden"  name="hargabeli[]" value ="` + $('#hargabeli').val() + `">  <input type="hidden"  name="dpps[]" value ="` + $('#dpp').val() + `">` + formatRupiah($('#dpp').val(), "Rp. ") + `</td>
										<td style="text-align:center" id=vat[]> <input type="hidden"  name="vat[]" value ="` + $('#vat').val() + `">` + formatRupiah($('#vatx').val(), "Rp. ") + `</td>
										<td style="text-align:center" id=qtyorder[]> <input type="hidden"  name="qtyorderx[]" value ="` + $('#qtyorder').val() + `"><input type="hidden"  name="qtyready[]" value ="` + $('#balance').val() + `">` + $('#qtyorder').val() + `</td>
										<td style="text-align:center" id=price[]> <input type="hidden"  name="pricex[]" value ="` + $('#subtotal').val() + `">` + formatRupiah($('#subtotalx').val(), "Rp. ") + `</td>
										<td style="text-align:center" id=disc[]> <input type="hidden"  name="discx[]" value ="` + $('#totaldiscount').val() + `">` + formatRupiah($('#totaldiscountx').val(), "Rp. ") + `</td>
				...
				baris += `
									<tr id=item` + $('#jmlitemx').val() + ` style="style="height:40px;text-align:center">
										<td>` + b + `</td>
											...
											<td style="text-align:center" id=hargaitem[]> <input type="hidden"  name="hargaitem[]" value ="` + $('#hargaitem').val() + `">` + formatRupiah($('#hargaitem').val(), "Rp. ") + `</td>
			<td style="text-align:center" id=dpp[]> <input type="hidden"  name="hargabeli[]" value ="` + $('#hargabeli').val() + `">  <input type="hidden"  name="dpps[]" value ="` + $('#dpp').val() + `">` + formatRupiah($('#dpp').val(), "Rp. ") + `</td>
											<td id=vat[]> <input type="hidden"  name="vat[]" value ="` + $('#vat').val() + `">` + formatRupiah($('#vatx').val(), "Rp. ") + `</td>
											...
											<td id=price[]> <input type="hidden"  name="pricex[]" value ="` + $('#subtotal').val() + `">` + formatRupiah($('#subtotalx').val(), "Rp. ") + `</td>
											<td id=disc[]> <input type="hidden"  name="discx[]" value ="` + $('#totaldiscount').val() + `">` + formatRupiah($('#totaldiscountx').val(), "Rp. ") + `</td>
											
											<td id=subtotal[]> <input type="hidden"  name="subtotalxx[]" value ="` + $('#grandtotal').val() + `">` + formatRupiah($('#grandtotalx').val(), "Rp. ") + `</td>
