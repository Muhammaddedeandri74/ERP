...
<div class="container-fluid py-2 mb-4" style="padding-left: 6vw;background-color : #3C2E8F">
	<p class="text-white font-weight-bold py-2 pl-2" style="font-size: 13px">PURCHASE / Purchase Order Book</p>
	<p class="text-white font-weight-bold pl-2 tu" style="font-size: 25px">PURCHASE ORDER BOOK</p>
</div>
								...
								<?php if ($data != "Not Found") { ?>
									...
									foreach ($data as $key) : ?>
										<?php if ($a % 2 == 0) { ?>
											...
											<td style="text-align:center"><?php echo number_format($key["totalpo"], 0, ".", ",") ?></td>
											<td style="text-align:center"><?php echo number_format($key["itemcount"], 0, ".", ",") ?></td>
											<td style="text-align:center"><?php echo number_format($key["qtypo"], 0, ".", ",") ?></td>
											<td style="text-align:center"><?php echo number_format($key["qtyin"], 0, ".", ",") ?></td>
...
<script language="javascript">
	$('#loadbtn').on("click", function(e) {
		window.location.href = "<?php echo base_url('reporting/bookpo') ?>?date1=" + $('#date1').val() + "&date2=" + $('#date2').val() + "&status=" + $('#status').val() + "";
	});
