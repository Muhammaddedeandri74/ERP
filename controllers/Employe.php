...
class Employe extends CI_Controller
{
		...
		foreach ($upload as $key) {
			...
			foreach ($data3 as $keyy) {
				...
				if (isset($key->SKU) && $key->SKU != '' && $key->SKU == $keyy["sku"]) {
					...
					$f["qty"]++;
					$f["subtotal"] += $key->{"Harga Dibayar"} * $key->{"Jumlah Produk"};
					$f["subtotals"] += $key->{"Harga Dibayar"} * $key->{"Jumlah Produk"};
					...
					$g["hargaitem"]  =   $key->{"Harga Dibayar"};
					$g["dpp"]  =  $hargabesih;
					$g["discx"] = 0;
					$g["hargabeli"] = $keyy["pricebuyitem"];
					$g["vat"] = $vat;
					$g["subtotalxx"] = $key->{"Harga Dibayar"} * $key->{"Jumlah Produk"};
	...
	function insertsalesorder()
	{
		...
		$vat = $this->input->post("vat");
		$dpp = $this->input->post("dpps");
		$price = $this->input->post("pricex");
		$disc     = $this->input->post("discx");
		$subtotal = $this->input->post("subtotalxx");
		// $froms = $this->input->post("froms");
		$cek = $this->Quotation->insertsalesorder($iduser, $typeso, $idcust, $noso, $nopes, $delivdate, $jasapengirim, $delivaddr, $delivfee, $idcurrency, $paymentmethod, $disnoms, $qty, $subtotals, $vatnom, $grandtotals, $iditem, $idwh, $nameitem, $sku, $qtyorder, $qtyready, $price, $disc, $subtotal, $noresi, $hargabeli, $hargaitem, $vat, $return, $datetrans, $dpp);
		print_r($cek);
		if ($from == "") {
			$this->session->set_flashdata('message', '<div class="alert alert-danger" role="alert">' . $cek . '</div>');
	...
	function insertsalesorderpending()
	{
		...
		$cek = $this->Quotation->insertsalesorderpending($iduser, $typeso, $idcust, $noso, $nopes, $delivdate, $delivaddr, $delivfee, $idcurrency, $paymentmethod, $disnoms, $qty, $subtotals, $vatnom, $grandtotals, $iditem, $idwh, $nameitem, $sku, $qtyorder, $qtyready, $price, $disc, $subtotal, $noresi, $hargabeli, $hargaitem, $vat, $return, $datetrans, $dpp);
	...
	function updatesalesorder()
	{
		...
		if ($notready == 0) {
			$cek = $this->Quotation->updatesalesorder($idso, $iduser, $typeso, $idcust, $noso, $nopes, $delivdate, $jasapengirim, $delivaddr, $delivfee, $idcurrency, $paymentmethod, $disnoms, $qty, $subtotals, $vatnom, $grandtotals, $iditem, $idwh, $nameitem, $sku, $qtyorder, $price, $disc, $subtotal, $noresi, $hargaitem, $hargabeli, $vat, $dpp);
		} else {
