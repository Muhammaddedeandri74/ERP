...
class CounterController extends CI_Controller
{
	...
	function stockhabis()
	{
		...
		$config['total_rows'] = $this->MasterData->Countgetitempagination($filter, $search, $dari, $sampai);
		$config['per_page']   = 50;
		$choice               = $config["total_rows"] / $config["per_page"];
		$config["num_links"]  = floor($choice);
		// Membuat Style pagination untuk BootStrap v4
		...
		// get item list
		$f["title"]          = "counter";
		$f["data"]           = $this->MasterData->getitemstockhabispagination($filter, $search, $dari, $sampai, $from, $end);
		$f["data1"]          = $this->MasterData->getoutsn();
